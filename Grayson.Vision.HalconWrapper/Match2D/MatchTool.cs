...
namespace Grayson.Vision.HalconWrapper.Match2D
{
    public static class MatchTool
    {
        ...
        public static Result<TemplateMatchResult[]> ApplyFindShapeModel(object nativeImage, int modelId, double minScore)
        {
        ...
        public static Result<TemplateMatchResult[]> ApplyFindNccModel(object nativeImage, int modelId, double minScore)
        {
            var hImg = nativeImage as HalconDotNet.HObject;
            if (hImg == null || !hImg.IsInitialized())
                return Result<TemplateMatchResult[]>.Fail("输入的图像句柄无效或未初始化");
