...
// 文件名: CaliperMeasureParam.cs
// 说 明: CaliperMeasure 节点参数（P2 §5.2 测量节点族）：
//        沿目标边缘布置多探针直线卡尺"找边点"（不做几何拟合），输出每个探针命中的
//        亚像素边缘点（Row/Col 数组 + 数量）。与 FitLine 的区别：FitLine 把点集拟合成
//        直线输出端点/残差；本节点只出点集，供下游"两点距离/边缘对间距/几何建构"等使用。
...
{
    public class CaliperMeasureParam : ParamBase
    {
        /// <summary>调参时预览实时刷新探针与边缘点。</summary>
        public override bool SupportsPreview => true;

        // ── 种子几何（可被上游 MidRow/MidCol 端口覆盖）──
        private double _seedMidRow;
        /// <summary>边缘段中点行（图像 Y；探针带中心）</summary>
        ...
        /// <summary>沿边探针数（3~15；每探针命中 1 个代表边缘点，点集长度 ≤ NumPoints）</summary>
