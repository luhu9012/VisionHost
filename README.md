...
<p align="center">
  <img src="docs/images/dashboard.png" width="880" alt="产线拓扑总览"/>
  <br/>
  <sub>产线拓扑总览 —— 多工位运行状态 / 产量 / 良率实时看板</sub>
</p>
...
| 能力域 | 工业场景 | 状态 | 说明 |
|---|---|---|---|
| **视觉引导定位 · 取放** | 视觉引导机器人（VGR）取放料 | 🟢 实机运行 | 成像→模板匹配→坐标换算→机械手联动取放；ETH / EIH 双布局 + 标定两级发布；复合工位实机节拍与 OK/NG 判定见下方截图 |
| **深度学习质检** | 缺陷检测 / 分类 / 分割 | 🟢 实机验证 | HALCON DL 一站式（DLTool → .hdl → C# 集成）；分割 / 分类任务实机图源验证 |
| **外观尺寸测量** | 尺寸 / 几何量测量 | 🟢 已实现 | 卡尺+拟合测量链 → 像素当量换算 mm → 测量值±公差判 OK/NG |
| **多相机复合工位** | 贴装 / 组装「取-校-放」 | 🟢 实机标定验证 | 固定上相机 + 固定下相机 + 眼在手上相机；坐标系统一与视轴镜像处理；九点标定实机自动采集 |
...
> 运行监控、标定、机械手调试等截图均来自**实机工位**（Basler / 海康相机 + Epson SCARA）。

### 运行监控（实机）

| 引导定位复合工位 · 实时运行 | 深度学习质检 · 实时推理 |
|---|---|
| ![引导定位实时监控](docs/images/station_monitor_guide.png) | ![DL 质检实时监控](docs/images/station_dl_monitor.png) |
| ST_002 复合工位实机运行：模板匹配得分实时叠加、节拍 / OK·NG 计数 / 良率一屏总览，右侧分级日志直击每个节点的执行与耗时 | 药片分割 DL 推理：缺陷区域分割叠加与置信度标注，HALCON 算子级日志与最近检测结果原图留档 |

### 流程编排与配方发布

| 节点图流程编辑器 | 配方与方案管理 |
|---|---|
| ![节点图流程编辑器](docs/images/flow_editor.png) | ![配方管理](docs/images/recipe_manager.png) |
| 节点参数面板、源图预览与控制台日志同屏：图像读取 → 通道拆分 → 形状匹配 → 标定转换，编辑态即运行态 | 配方按工程分组管理、两级审批（草稿/已审批）、工艺参数集与硬件设备需求清单，一键跳转流程编辑器 |

### 视觉资产：模板与标定（实机）

| 视觉模板工作台 · 真机取流 | 标定向导 · 配准度门禁 |
|---|---|
| ![模板工作台](docs/images/template_workbench.png) | ![标定向导](docs/images/calibration_wizard.png) |
| Basler 相机真机取流 + 实时形状匹配（得分 0.992）；模板创建五步向导，Shape/Ncc 匹配、模板参数与学习策略配置化 | 向导六步（锁定相机与运动轴 → 设中心点 → 提取特征 → 拉点采集 → 拟合计算与几何自检 → 验收发布），配准度评分 99/100、特征位置 0.6px 的量化门禁 |
| ![九点标定自动采集](docs/images/calibration_ninepoint.png) | ![标定中心](docs/images/calibration_center.png) |
| **九点标定 · 全自动走位采集**：Basler + Epson 联动，逐点匹配评分 9/9（98~99 分）自动采集，像素误差与机器人位姿实时回显，一键全自动走位 | **标定中心 · 方案与档案管理**：标定方案状态机（启用/待发布）、标定档案管理、下相机九点向导入口，多方案多相机统一管理 |

### 工位装配与设备调试（实机）

| 工位工程工作台 | 机械手调试 · 真机 IO 验证 |
|---|---|
| ![工位工作台](docs/images/station_workbench.png) | ![机械手调试](docs/images/robot_debug.png) |
| 装机明细 / 受力-逻辑映射 / 联合运行 / 任务模板四步装配，硬件设备绑定与装配检查一体化 | Epson SCARA 真机联调：四轴实时坐标与单轴步进、真空阀 IO 测试（双吸嘴）、workspace 校验 24/24 项通过、可达域与九点预演 |

### 数据与运维

| 本地追溯与防错 | 驱动插件管理 |
|---|---|
| ![追溯](docs/images/traceability.png) | ![插件管理](docs/images/plugin_manager.png) |
| 检测记录全字段可查（工位/批次/配方/结果/耗时/错误信息），按工位与批次检索、实测工单、导出 | 相机 / 机械手 / 运动控制 / PLC / 协议 / 推理全家族插件自动扫描注册，即插即用 |

> 更多界面截图见 [`docs/images/`](docs/images/)（任务模板中心、工位工作台等）。
...
- 🟢 **已实现**：标定向导 v2（含九点标定全自动走位采集）、配方两级发布、流程编排、模板源图留档、设备插件体系、DL 质检、外观测量、任务模板中心、引导定位复合工位实机运行；
- 🟡 **进行中**：多相机复合工位（上相机 + 下相机 + EIH）标定链路收尾与业务模板沉淀；
...
本项目未附带开源许可证。代码与文档用于技术交流与能力展示，如需合作或商用请与作者联系。
