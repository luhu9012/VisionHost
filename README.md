...
| **多相机复合工位** | 贴装 / 组装「取-校-放」 | 🟢 已实现 | 固定上相机 + 固定下相机 + 眼在手上相机；坐标系统一与视轴镜像处理 |
...
| 单工位实时监控 |
|---|
| ![DL 质检实时监控](docs/images/station_dl_monitor.png) |
| DL 推理实时画面 + 置信度标注 + OK/NG 判定，右侧节点级运行日志可直接看到 HALCON 算子执行链路，最近检测结果带原图留档 |
...
| 自研节点图引擎：图像读取 → 通道拆分 → 形状匹配 → 标定转换，编辑态即运行态，控制台实时输出节点执行日志 | 配方按工程分组管理、两级审批（草稿/已审批）、工艺参数集与硬件设备需求清单，一键跳转流程编辑器 |
...
| 模板创建五步向导（选图→学习→视角导入→生成配置→参数创建），Shape/Ncc 匹配、模板参数与学习策略配置化 | 标定方案状态机（启用/待发布），向导六步：锁定相机与运动轴 → 设相机中心点 → 提取特征 → 拉点采集 → 拟合计算与几何自检 → 验收发布，含配准度评分门禁 |
...
| 装机明细 / 受力-逻辑映射 / 联合运行 / 任务模板四步装配，硬件设备绑定与装配检查一体化 | Epson SCARA 四轴实时坐标、单轴步进，可达域与九点预演（workspace map + 碰撞与限位检查 + 逐点试运行） |
...
> 更多界面截图见 [`docs/images/`](docs/images/)（任务模板中心、标定方案列表、DL 质检监控等）。
